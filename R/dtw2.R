...
#' variables, and that univariate series will produce the same LCM regardless of the L-norm used. After the
#' warping path is found by DTW, the final distance is calculated as the L-norm of all \eqn{(i,j)}
#' elements of the LCM that fall on the warping path.
#'
#' The \code{\link[dtw]{dtw}} function allows changing the norm by means of its \code{dist.method} parameter,
#' but it only uses it when creating the LCM, and not when calculating the final aggregated cost, i.e. the
#' DTW distance.
#'
