...
DBA <- function(X, center = NULL, max.iter = 20,
     ...
     ## pre-allocate matrices for the calculation of the local costs matrices
     ...
     while(iter<=max.iter) {
          ...
          xg <- mapply(X, M, SIMPLIFY = FALSE, FUN = function(x, xm) {
               # on average, I don't know if this approach is more efficient than just calling dtw with 2 arguments
