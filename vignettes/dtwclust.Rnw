...
Internally, the DTW alignment can be computed by the \pkg{dtw} package or with the included \code{dtw_basic} function. The latter should be faster, but the alignments will not be identical if a \textit{unique} optimum warping path does not exist, which can happen when several elements of the LCM have the same value (e.g. zero).
