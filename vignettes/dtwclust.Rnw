...
Algorithms for hierarchical clustering can be \textbf{agglomerative} or \textbf{divisive}, with the former being much more common than the latter \citep{hastie2009}. In agglomerative procedures, every element of the data starts in its own cluster, and elements are grouped together sequentially based on the similarity measure until all elements are contained in a single cluster. Divisive procedures do the exact opposite, starting with all data in one cluster and dividing it until each element is in a \textit{singleton}. Both strategies suffer from a lack of flexibility, because they can't perform adjustments once a split or merger has been done.
