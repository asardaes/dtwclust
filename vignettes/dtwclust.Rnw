...
%\VignetteDepends{TSclust, TSdist, doParallel}
...
(e.g. DTW in \pkg{dtw} \citep{giorgino2009} or Keogh's DTW lower bound in \pkg{TSdist} \citep{tsdist}),
the implementations in \dtwclust{} are optimized for speed,
since all of them are implemented in \proglang{C++} and have custom loops for computation of cross-distance matrices,
