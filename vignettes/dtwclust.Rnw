...
\blankfootnote{This manuscript corresponds to \dtwclust{} version \Sexpr{packageVersion("dtwclust")}.}
...
When it comes to static data, i.e., if the values do not change with time \citep{liao2005},
clustering methods are usually divided into five major categories:
...
Time-series are a common type of dynamic data that naturally arises in many different scenarios,
such as stock data,
medical data,
...
In this context, dimensionality of a series is related to time, and it can be understood as the length of the series.
...
In most cases, algorithms developed for time-series clustering take static clustering algorithms and either modify the similarity definition or the prototype extraction function by an appropriate one,
...
where the former usually deals with grouping incoming data streams on-the-go (see, e.g., \citet{rakthanmanon2011}),
while the latter deals with data that no longer change.

In this context, it is common to utilize the Dynamic Time Warping (DTW) distance \citep{aghabozorgi2015} as dissimilarity measure.
...
especially if the data complexity increases (which can happen really quickly in case of time-series),
or some of the more computationally expensive algorithms are used.

The choice of time-series representation,
preprocessing,
and clustering algorithm has a big impact in performance with respect to cluster quality and execution time.
...
As such, it is desirable to have a common platform on which clustering algorithms can be tested and compared against each other.
...
something that many other packages lack.
...
Please refer to \cref{app:notes} for further information.
...
Thus, many other distance measures tailored to time-series have been developed in order to overcome this limitations as well as other challenges associated with the structure of time-series,
...
Refer to \cref{app:custom-dist} for examples on how to use the \pkg{proxy} package with \dtwclust{} and exploit custom distance measures,
as well as those defined in other \R{} packages.
...
At the core,
DTW is a dynamic programming algorithm that compares two series and tries to find the optimum warping curve between them under certain constraints,
...
In this instance, the initial and final points of the series must match, but other points may be warped in time in order to find better matches.
...
The \pkg{dtw} package \citep{giorgino2009} includes a \proglang{C} implementation of the dynamic programming step of the algorithm, which should be very fast;
...
The DTW distance is considered a more robust distance measure for time-series \citep{wang2013},
and can potentially deal with series of different length directly.
...
It is clear that the choice of $l_p$ norm comes into play twice during the DTW the algorithm,
...
but appropriately uses the $l_2$ norm in the second step.
...
As final comments, it should be noted that the DTW distance does not satisfy the triangle inequality,
...
If a window constraint is used with \dtwclust{}, a slanted band is employed.
...
They have been exploited when indexing time-series databases, classification of time-series, clustering, etc. \citep{keogh2005, begum2015}.
...
It must be noted that the LBs are only defined for series of equal length and,
similar to DTW,
are not symmetric regardless of the $l_p$ norm used to compute them.
Also note that the choice of step pattern affects the value of the DTW distance,
changing the tightness of a given LB.

One crucial step when calculating the LBs is the computation of the so-called envelops.
These envelops require a window constraint,
and thus are dependent on both the type and size of the window.
Based on these, a running minimum and maximum are computed and, respectively, a lower and upper envelop are generated.
\Cref{fig:envelop-plot} depicts a sample time-series with its corresponding envelops for a Sakoe-Chiba window of size 15.
...
a copy of the reference series is created;
denote this copy with $H$.
The envelops for the other series (the query) are computed:
if $y$ is the query, then its upper and lower envelop (UE and LE respectively) are compared against $x$,
so that the values of $H$ are updated according to \cref{eq:lb-envelop}.
...
\citet{lemire2009} developed a streaming algorithm to calculate the envelops using no more than $3n$ comparisons when using a Sakoe-Chiba window.
This algorithm is implemented in \dtwclust{} using the \proglang{C++} programming language,
ensuring an efficient calculation.

\code{LB_Keogh} requires the calculation of one set of envelops for every pair of series compared,
whereas \code{LB_Improved} must calculate two sets of envelops for every pair of series.
If the LBs must be calculated between several time-series,
some envelops can be reused when a given series is compared against many others.
This optimization is included in the LB functions registered with \pkg{proxy} by \dtwclust{}.

The function \code{dtw_lb} included in \dtwclust{} (and the corresponding version registered with \pkg{proxy}) leverages \code{LB_Improved} in order to find nearest neighbors faster.
...
It continues this process iteratively until no changes in the nearest neighbors are detected. See \cref{app:dtw-lb} for specific examples.
...
Additionally, a considerably large dataset would be needed before the overhead of DTW becomes much larger than that of \code{dtw_lb}.

\subsection{Shape-based distance}
\label{sec:sbd}

The shape-based distance (SBD) was recently proposed as part of the \kshape{} clustering algorithm \citep{paparrizos2015};
this algorithm will be further discussed in \cref{sec:shape} and \cref{sec:kshape}.
SBD is based on the cross-correlation with coefficient normalization (NCCc) sequence between two series,
and it is thus sensitive to scale,
...
It is shown that the DTW distance can be understood as the cost associated with the minimum alignment as shown in \cref{eq:dtw-kernel},
...
although it is slightly more expensive to compute in comparison to DTW.
...
A very important element of time-series clustering has to do with the calculation of so-called time-series prototypes.
...
however we will prefer the term ``prototyping'' in this paper.
...
it is debatable what exactly constitutes an average time-series,
...
As of version 3.2.1, deactivating precomputation falls back to using a sparse matrix that is updated on each iteration,
and also across repetitions if done sequentially.
For small datasets, calculating the whole matrix beforehand is still probably faster.
...
so for each time-point in the centroid, all the corresponding values from all series in $C$ are grouped together according to the DTW alignments,
and the mean is computed for each centroid point using the values contained in each group.
This is then iteratively repeated until a certain number of iterations are reached,
or until convergence is assumed.

One way in which DBA can be optimized is in its usage of random access memory (RAM).
Since the series used on each iteration do not change,
the LCMs generated by DTW always have the same size,
so the required memory can be allocated once and re-used on each iteration.
In order to ensure that this is the case,
the \dtwclust{} implementation updates the LCMs when using DBA by means of \proglang{C} code.

DBA is much more computationally expensive due to all the DTW calculations that must be performed.
...
starting with all data in one cluster and dividing it until each member is in a singleton.
...
In the latter case, several cuts can be made,
...
First, $k$ centroids are randomly initialized, usually by choosing $k$ objects from the dataset at random;
...
Additionally, when PAM centroids are used,
the function simply keeps track of the data indices,
how they change and which ones are chosen as cluster centroids,
...
since the cluster centroids are always elements of the data.
However, this algorithm is deterministic,
depending on the value of a cutoff distance ($d_c$).

The algorithm first uses the DTW distance's upper (Euclidean distance) and lower (\code{LB_Keogh}) bounds to find series with many close neighbors (in DTW space).
Anything below $d_c$ is considered a neighbor.
Aided with this information, the algorithm then tries to prune as many DTW calculations as possible in order to accelerate the clustering procedure.
The series that lie in dense areas (i.e., that have lots of neighbors) are taken as cluster centroids.
For a more detailed explanation of each step in the algorithm,
please refer to \citet{begum2015}.

The algorithm relies on the DTW bounds,
...
In crisp partitions, each member of the data belongs to only one cluster,
and clusters are mutually exclusive.
By contrast, fuzzy clustering creates a fuzzy or soft partition in which each member belongs to each cluster to a certain degree.
...
(note that some of the literature use the transposed version of $u$).
...
and doing so in \dtwclust{} is straightforward, since it only entails registering the corresponding distance function with \pkg{proxy}.
...
and it is common to first apply a transformation to the data and cluster in the resulting space.
For instance, \citet{durso2009} used the autocorrelation function to extract a certain amount of coefficients from time-series,
...
This centroid function was implemented in \dtwclust{} in version 3.1.0.
...
Each worker may require additional RAM, and \R{} can only work with data that is loaded on RAM,
...
When many repetitions are specified directly to \dtwclust{},
...
This chunking is also performed by the distance functions used by \dtwclust{} which are registered with \pkg{proxy},
specifically: \code{LB_Keogh}, \code{LB_Improved}, \code{SBD}, \code{dtw_basic}, \code{dtw_lb} and \code{GAK}.

The included implementation of the TADPole and DBA algorithms also try to create chunks when calculating DTW distances.
The amount of calculations needed should be relatively small,
so parallelization may not always be worth it in these cases.
...
The discussion here is based on \citet{arbelaitz2013},
which provides a much more comprehensive overview.
...
In many cases, these CVIs can be used to evaluate the result of a clustering algorithm regardless of how the clustering works internally,
or how the partition came to be.
The Silhouette index is an example of an internal CVI,
whereas the Variation of Information \citep{meilua2003} is an external CVI.

Several of the best-performing CVIs according to \citet{arbelaitz2013} are implemented in \dtwclust{} in the \code{cvi} function,
although other indices can be implemented by the user.
However, in the original definition of many internal CVIs,
...
making it compatible with many of the existing CVIs,
...
As of \dtwclust{} version 3.0.0, support for the \pkg{clue} package functions is included.
...
any suitable distance function can be used (see \cref{app:custom-dist}).
...
As previously mentioned, most time-series clustering algorithms use existing functions and only modify the distance and centroid functions to an appropriate one.
...
In this manuscript, a general overview of the current state of the art in time-series clustering was provided.
...
(respectively: distance calculation, hierarchical clustering, cluster evaluation),
...
so we hope that, if time-series clustering is required,
...
The original main clustering function in \dtwclust{} has the same name, i.e., \code{dtwclust}.
As of version 3.1.1, a new main function \code{tsclust} was added,
which is hopefully more coherent in general.
For completeness, comments about both are given below.
Both return objects of type \code{S4}, which are one way in which \R{} implements classes.
...
With regard to the original \code{dtwclust} function,
all documented information about the slots of the returned objects can be found by typing \code{?"dtwclust-class"} in the console.
The associated methods can be found with \code{?"dtwclust-methods"}.
Note that the \code{dtwclust} class contains the \code{S3} class \code{hclust} as a superclass.
This can lead to some unexpected behavior,
such as \code{is.list(new("dtwclust"))} being \code{TRUE}.
Many control parameters can be modified by using the \code{control} argument,
which should have a \code{dtwclustControl} class.
A named list is also accepted.
The documentation of all control parameters,
as well as their default values,
can be accessed by typing \code{?"dtwclustControl"} in the console.

For the new \code{tsclust} function,
the documentation of the slots for the objects returned can be found with \code{?"TSClusters-class"},
and the methods with \code{?"tsclusters-methods"}.
Controls are now set via intermediary functions whose documentation can be found with \code{?"tsclust-controls"}.

The random number generator of \R{} is set to L'Ecuyer-CMRG when \dtwclust{} is attached in an attempt to preserve reproducibility.
...
all the distances mentioned in this manuscript are registered with \pkg{proxy} when \dtwclust{} is attached to \R{}.
...
Internally, \dtwclust{} coerces all provided data to a list and,
as of version 2.3.0, it parses both matrices and data frames row-wise.
...
The function \code{compare_clusterings} and some helper functions were added in version 3.2.0 to test many different clustering configurations in a convenient way.
...
As a first example,
the autocorrelation-based distance included in the \pkg{TSclust} package \citep{montero2014} is registered with \pkg{proxy} so that it can be used either directly,
or with \dtwclust{}.
...
In the following example, the nearest neighbors in DTW space of the first 5 time-series are found in two different ways,
...
In the following call to \dtwclust{},
...
Therefore, the data is \textit{z}-normalized by means of the \code{zscore} function.
...
Using the autocorrelation function overcomes the problem of time-series with different length.
...
According to the Silhouette index,
6 is the optimum number of clusters,
whereas the CH index suggests using only 2,
but the DB* and Dunn indices suggest the true number of clusters,
namely 4.
