...
\section{Registering a custom distance with proxy}
\label{app:custom-dist}

This sample takes the autocorrelation-based distance included in the \pkg{TSclust} package \citep{montero2014} and registers it with \pkg{proxy}.
...
# Note that subsetting with a single brackets preserves the list format
