...
As of version 3.1.1, the main clustering function is \code{tsclust}.
It returns objects of type \code{S4}, which are one way in which \R{} implements classes.
...
Controls are set via intermediary functions whose documentation can be found with \code{?"tsclust-controls"}.
