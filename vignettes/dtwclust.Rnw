...
envelopes <- compute_envelop(CharTraj[[2L]], window.size = 15)
...
The envelopes for the other series (the query) are computed:
if $y$ is the query, then its upper and lower envelope (UE and LE respectively) are compared against $x$,
