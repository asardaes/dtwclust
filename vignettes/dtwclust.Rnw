...
As previously mentioned, most time-series clustering algorithms use existing functions and only modify the distance and centroid functions to an appropriate one. On the other hand, a lot of work has also been devoted to time-series representation, but performing data transformations can be done independently from \dtwclust{}, or provided as a preprocessing function if desired, which might be required if the \code{predict} generic is to be used.
