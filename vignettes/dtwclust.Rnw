...
This algorithm is implemented in \dtwclust{} as a standalone function, which the main clustering function can also call, serving as a wrapper.
...
If we choose the value of $k = 4$, we could then compare results among different random repetitions with help of the \pkg{clue} package.
