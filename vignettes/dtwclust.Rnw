...
fam <- new("dtwclustFamily",
           dist = "dtw_lb",
           control = ctrl,
           allcent = function(dummy) {
               data[tadp$centroids[1L]]
           })

cldist <- fam@dist(data[1L:20L],
                   centroids[tadp$cl],
                   pairwise = TRUE)
cldist <- as.matrix(cldist)
size <- as.vector(table(tadp$cl))
clusinfo <- data.frame(size = size, av_dist = 0)
clusinfo[clusinfo$size > 0L, "av_dist"] <-
    as.vector(tapply(cldist[ , 1L], tadp$cl, mean))
...
cbind(cvi(res, CharTrajLabels[1L:20L]))
@
