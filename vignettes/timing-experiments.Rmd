...
```{r cent-dba-plot, fig.height=20, fig.cap="*The facets' rows indicate the DBA version and the window size used, 'byS' means 'by series' and 'byV' means 'by variable' (see DBA documentation). The facet's columns indicate how many threads were used. Note the different vertical scales for each row of the facets.*"}
...
but it is not automatically used by default
(see its documentation);
for this experiments parallelization was explicitly tested.
...
so the fact that they all have access to all threads makes it a fair comparison.
...
levels(ggdf$variable) <- c("DTW (Full Pre-Computation)", "DTW (Sparse Matrix)", "DTW + LBI")
ggplot(ggdf,
       aes(x = num_series,
           y = value,
           colour = window_size)) +
...
ggdf <- reshape2::melt(partitional_results$dtwlb_vs_dtwbasic$dba, 
                       id.vars=c("num_series", "k", "window_size"))
levels(ggdf$variable) <- c("DTW", "DTW + LBI")
ggplot(ggdf,
       aes(x = num_series,
           y = value,
           colour = window_size)) +
