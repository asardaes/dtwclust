...
the `x` axis contains the total number of series in the cross-distance matrix,
but the color is mapped to the number of *columns* in said matrix.
This is relevant in this case because of the envelops that need to be computed when calculating the lower bounds.
These are computed for the series in `y`,
which end up across the columns of the distance matrix.
The applied optimization consists in calculating the envelops for `y` only once,
...
which indicates that DTW cannot be optimized much more,
...
GAK also uses a matrix to save logarithms during the intermediate calculations,
...
Also note that this implementation of DBA allocates the LCM used for backtracking only once;
...
There are currently 3 different strategies to apply PAM centroids implemented:
...
Even though **the remarks apply to any distance**,
...
```{r clust-part-dtw-pam-reps-plot, fig.height=5, fig.cap="*The window size case fixed at 20 for these experiments.*"}
...
but by the time we use 10 repetitions,
...
so using sparse matrices or, for DTW distance, using `dtw_lb` is not a very good choice.
