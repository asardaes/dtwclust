...
The distance and prototyping experiments were re-run with `R` v3.4.3 and `dtwclust` v5.1.0.9000.
The rest were run using `R` v3.4.1 and `dtwclust` v4.0.3.
...
id_gg <- grepl("^sbd", dist_single_results$distance)
gg <- ggplot(dist_single_results[id_gg,], aes(
    x = series_length,
    y = median_time_us,
    group = 1
...
```{r dist-single-sdtw, fig.height=4.5}
id_gg <- grepl("^sdtw", dist_single_results$distance)
gg <- ggplot(dist_single_results[id_gg,], aes(
    x = series_length,
    y = median_time_us,
    group = 1
...
The increase caused by considering multivariate time-series is not negligible.

### Triangular global alignment kernel

```{r dist-single-gak, fig.height=6}
id_gg <- grepl("_gak_", dist_single_results$distance)
gg <- ggplot(dist_single_results[id_gg,], aes(
    x = series_length,
    y = median_time_us,
    group = window_size,
...
its helper matrix is allocated only once during the calculation.
In this case,
we look only at the univariate version.

```{r dist-multiple-sdtw-plot, fig.height=8, fig.cap="*The facets' columns indicate the number of parallel workers, whereas the rows indicate the length of the series being considered (all series had the same length for each case). All times are in milliseconds.*"}
id_gg <- grepl("^sdtw_[um]", dist_multiple_results$distance)
ggplot(dist_multiple_results[id_gg,],
       aes(x = num_total,
           y = median_time_ms)) +
    geom_point(size = 3) +
...
```{r cent-sdtwc-plot, fig.height=8, fig.cap="*The facets' rows indicate the length of the considered series. Note the different vertical scales for each row of the facets.*"}
id_gg <- grepl("^sdtw_", cent_results$cent)
ggplot(cent_results[id_gg,],
       aes(x = num_series,
           y = median_time_ms)) +
    geom_line() +
...
Surprisingly, the multivariate tests were faster than the univariate ones.
Unfortunately, the optimization procedure is considerably slow for data with high dimensionality,
making the centroid calculation orders of magnitude slower than the other options.
