...
also be used with `dtw_lb` and `DBA`. It will now be used by default, but in the case of `DBA`, the results **will** be different because the backtracking algorithm differs with respect to the one in the `dtw` package.
